(ns travelport.main
  "Kotodama WASM entrypoint for the Travelport clean-room actor (L4) — Clojure port.
  ...
  facts (`travelport.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "travelport")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"travelport.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "travelport-compat" :tier tier :entities entities} 200])
...
(defn start! [] :travelport-compat/ready)
